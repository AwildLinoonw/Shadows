...
struct PSInput
{
	...
	float4 colour:COLOUR0;
	float4 UntransformedPos:POSITION;
...
void PSMain(const PSInput input, out PSOutput output)
{
	...
	float4 LightPos = mul(input.UntransformedPos, g_shadowMatrix);
	...
	if (LightPos.w >= 0)
	{
		...
		float4 Sample = g_shadowTexture.Sample(g_shadowSampler, float2(LightPos.x, 1 - LightPos.y));
		// If it is then alpha blend between final colour and shadow colour
		output.colour = lerp(input.colour, g_shadowColour, Sample.r);
	}
	else
	{
